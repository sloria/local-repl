...
describe('loadContext', () => {
  it('should load values', done => {
    p.loadContext([{name: 'foo', value: 42}]).then(context => {
  ...
  it('should load installed modules', done => {
    p.loadContext([{name: 'foo', module: 'lodash'}]).then(context => {
  ...
  it('should load local modules', done => {
    p
      .loadContext([{name: 'foo', module: './test/local-module'}])
      .then(context => {
  ...
  it('should throw an error if both module and value passed', done => {
    p.loadContext([{name: 'foo', value: 42, module: 'lodash'}]).catch(err => {
      expect(err.message).to.equal(
        'Context entry for "foo" cannot define both "module" and "value".'
      );
      done();
    });
  });
  ...
  it('should throw an error if neither module nor value are passed', done => {
    p.loadContext([{name: 'foo'}]).catch(err => {
      expect(err.message).to.equal(
        'Context entry must contain either "module" or "value".'
      );
      done();
    });
  });
  ...
  it('should error if name not provided', done => {
    p.loadContext([{value: 42}]).catch(err => {
      expect(err.message).to.equal(
        '"name" is required for each context entry.'
      );
      done();
    });
  });
  ...
  it('should load promises with context array', done => {
    ...
    p.loadContext([{name: 'meaning', value: promise}]).then(context => {
  ...
  it('should load promises with context object', done => {
    const promise = Promise.resolve(42);
    p.loadContext({meaning: promise}).then(context => {
      expect(context).to.equal({meaning: 42});
      done();
    });
  });
  ...
  it('should handle rejected promises', done => {
    ...
    p.loadContext({err: promise}).catch(err => {
  ...
  it('should allow prompts to be defined as a function', done => {
    p
      ...
      .then(result => {
        ...
        expect(result.promptFunc(result.context, result.package)).to.equal(
          'TEST foo > '
        );
        done();
      });
